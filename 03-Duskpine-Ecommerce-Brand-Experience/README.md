# 🌙 Duskpine — Quiet, Naturally

> **UX/UI Case Study | Brand Identity | E-commerce Experience | Social Media Campaign**

---

## 📖 Overview

Duskpine is a conceptual direct-to-consumer (DTC) wellness brand offering handcrafted natural beeswax noise-cancelling earplugs. This project explores the complete UX/UI design process, from building a premium brand identity to designing a responsive e-commerce landing page, a streamlined checkout experience, and cohesive social media marketing assets.

The primary objective was to create a user-centred digital experience that educates visitors, builds trust, and encourages purchase through intuitive design and compelling visual storytelling.

---

## 🎯 Project Objectives

* Design a premium and memorable brand identity.
* Create a responsive, conversion-focused landing page.
* Design a simple and frictionless checkout experience.
* Maintain visual consistency across all digital touchpoints.
* Develop social media assets aligned with the brand identity.

---

## 🧩 Design Process

* Brand Identity
* User Experience Strategy
* Landing Page Design
* Checkout Experience
* Design System
* Responsive Design
* Social Media Campaign

---

## 📂 Project Structure

```text
01-Brand-Identity
02-Landing-Page-&-Checkout
03-Social-Media-Campaign
```

---

# 🎨 Deliverables

## 1. Brand Identity

**🎨 Figma Design**

https://www.figma.com/design/6Bcs45glLthT7hd3GYXw1O/Duskpine---Quiet--Naturally?node-id=0-1&t=vN3EErVRC804DDrW-1

<img width="1440" height="1024" alt="01-Brand-Identity" src="https://github.com/user-attachments/assets/2f8254bc-c141-465a-8ad2-cb2abeb358e0" />

---

## 2. Landing Page Design

**▶️ Interactive Prototype**

https://www.figma.com/proto/6Bcs45glLthT7hd3GYXw1O/Duskpine---Quiet--Naturally?node-id=13-9&viewport=69%2C131%2C0.27&t=QVmQykId45LN2rp3-1&scaling=min-zoom&content-scaling=fixed&page-id=1%3A4

<img width="1440" height="4724" alt="Landing Page" src="https://github.com/user-attachments/assets/b422afd2-6134-4b85-9579-3f103dc7f463" />

---

## 3. Checkout Experience

**▶️ Interactive Prototype**

https://www.figma.com/proto/6Bcs45glLthT7hd3GYXw1O/Duskpine---Quiet--Naturally?node-id=13-110&viewport=-191%2C100%2C0.23&t=HnXpzHdXSl5pb3aD-1&scaling=min-zoom&content-scaling=fixed&page-id=1%3A4

<img width="1440" height="2006" alt="Checkout Page" src="https://github.com/user-attachments/assets/7128234f-f2c0-4441-a166-f3b10f958a99" />

---

## 4. Social Media Campaign

**🎨 Figma Design**

https://www.figma.com/design/6Bcs45glLthT7hd3GYXw1O/Duskpine---Quiet--Naturally?node-id=13-8&t=vN3EErVRC804DDrW-1

<img width="1200" height="675" alt="Twitter Post" src="https://github.com/user-attachments/assets/c071e82e-e12e-412e-ab03-d92b9f4b6fec" />

---

## 📄 UX Case Study

**Complete UX/UI Case Study (PDF)**

> *Upload your PDF to this repository and replace the link below.*

```text
/Duskpine-UX-UI-Case-Study.pdf
```

---

## ✨ Highlights

* Premium Brand Identity
* User-Centred Design
* Responsive E-commerce Experience
* Conversion-Focused UX
* Design System
* Checkout Experience
* Social Media Campaign

---

## 🛠 Tools

* Figma
* FigJam
* Canva

---

## 👩‍💻 Designer

**Jananie Logendra**

UI/UX Designer

---

## 🙏 Thank You

Thank you for taking the time to explore this project.

This case study reflects my approach to combining user-centred design, branding, and conversion-focused UX to create meaningful digital experiences.

I welcome feedback, collaboration opportunities, and professional connections.

---

*This project was created as an independent UX/UI case study for portfolio purposes.*
