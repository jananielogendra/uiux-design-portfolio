# 🤖 Base360 — AI Conversations, Reimagined

> **UX/UI Case Study | AI SaaS Landing Page | Responsive Web Design | Conversion-Focused Experience**

---

## 📖 Project Overview

Base360 is an AI-powered communication platform that unifies customer conversations across multiple channels into one intelligent system.

This UX/UI case study showcases the complete design process—from product research and UX strategy to high-fidelity interface design, responsive adaptation, and interactive prototyping. The primary goal was to simplify a feature-rich AI platform into a clear, engaging, and conversion-focused landing page that effectively communicates value and builds user trust.

---

## 🎯 Project Objectives

* Simplify a complex AI product through clear storytelling.
* Design a high-converting SaaS landing page.
* Build user trust through product previews and social proof.
* Develop a scalable and reusable design system.
* Deliver a seamless responsive experience across multiple devices.

---

## 🧩 UX Design Process

This project follows a structured UX design workflow:

* Product Understanding
* Problem Statement
* Business Goals
* Target Audience
* Competitor Research
* User Journey
* Information Architecture
* Wireframes
* Visual Direction
* Design System
* High-Fidelity UI Design
* Responsive Design
* Key Takeaways

---

## ✨ Design Highlights

* Modern AI SaaS Interface
* Conversion-Focused Landing Page
* Strong Visual Hierarchy
* Responsive Web Experience
* Scalable Design System
* Interactive Components & Hover States
* User-Centred Design
* Clean Information Architecture

---

## 🛠 Tools Used

* Figma
* FigJam
* Google Fonts (Inter)
* UX Research
* Wireframing
* Interactive Prototyping

---

## 📂 Project Structure

* UX Case Study (PDF)
* Product Research
* Competitor Analysis
* User Journey
* Information Architecture
* Wireframes
* Design System
* Final UI Screens
* Responsive Designs
* Interactive Prototype

---

# 🎨 Project Preview

## 💻 Desktop Landing Page

<img width="720" height="3250" alt="13-Final-Landing (Desktop)-Page" src="https://github.com/user-attachments/assets/b40989e9-7397-48d0-8365-ee5ac2b7a376" />

---

## 📱 Mobile Responsive Design

<img width="1608" height="9496" alt="Mobile Responsive" src="https://github.com/user-attachments/assets/d7212a8b-55a5-445f-90fc-ac2a77336746" />

---

# 📄 UX Case Study

**Complete UX/UI Case Study (PDF)**

📥 **[View Complete UX/UI Case Study (PDF)](https://github.com/jananielogendra/uiux-design-portfolio/raw/main/02-Base360-AI-SaaS-Landing-Page/16-Base360-AI-SaaS-Landing-Page-UX-Case-Study.pdf)**

---

# 🎨 Figma Files

## 🎨 Design File

🔗 [View Complete Design File](https://www.figma.com/design/KYkOMocvKKKb3FP3r4QGLn/Base360---AI-Conversations--Reimagined?node-id=0-1&t=EVsKPWLxO2Mxzgc7-1)

---

## ▶️ Interactive Prototype

🔗 [View Interactive Prototype](https://www.figma.com/proto/KYkOMocvKKKb3FP3r4QGLn/Base360---AI-Conversations--Reimagined?node-id=6-1213&p=f&viewport=360%2C221%2C0.17&t=fbuciBdJzV0TcI3C-1&scaling=scale-down&content-scaling=fixed&page-id=5%3A1209)

---

# 🎥 Presentation

## 🎬 Loom Walkthrough

🔗 [Watch Project Walkthrough](https://www.loom.com/share/8b1e0f43ec82435099889fcf00ac8908)

---

## 📈 Outcome

This project demonstrates a complete end-to-end UX/UI design process, transforming a technically complex AI platform into a modern, intuitive, and conversion-focused digital experience. Through strategic storytelling, responsive design, and a scalable design system, the final solution improves product clarity, strengthens user trust, and effectively communicates the platform's value to potential customers.

---

## 👩‍💻 Designer

**Jananie Logendra**

UI/UX Designer | Project Management Enthusiast | Business Analysis

📧 **Email:** [jananielogendra05@gmail.com](mailto:jananielogendra05@gmail.com)

💼 **LinkedIn:** [www.linkedin.com/in/jananie-logendra-0395jl](http://www.linkedin.com/in/jananie-logendra-0395jl)

---

## 🙏 Thank You

Thank you for taking the time to explore this project.

This case study reflects my approach to solving complex product challenges through user-centred design, strategic thinking, and modern interface design.

I welcome feedback, collaboration opportunities, and professional connections.

